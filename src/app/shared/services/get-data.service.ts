...
export class GetDataService {
  ...
  initData() {
    ...
    if (cached) {
      ...
      let tmp = this.http.get("https://restcountries.com/v3.1/all")
  ...
  getCountryFromQuery(name: string): Observable<any> {
    return this.http.get(`https://restcountries.com/v3.1/name/${name}`).pipe(map((d: any) => d));
  }
  ...
  getCountryUsingCode(code: string): Observable<CountryData> | undefined {
    ...
    return this.countries?.pipe(
      map(countries => countries.find(country => country.cca3 == code))
